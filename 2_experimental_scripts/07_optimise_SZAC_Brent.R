# ################################################################
# objective:   optimse the SZAC parameter using Brent method
...
output_path = '/datasets/work/d61-af-soilmoisture/work/himawari/ZAC/Brent/'
...
for (k in 1:length(Dates)){
    ...
    anu_lst = paste0(processed_path, 'ANU/', format(Dates[k], '%Y/%m/%d/%Y%m%d0'), time_range, '00_AHI_ANU_LSTv1.0_AusSubset.tif')
...
# zenith angle-based calibration
ZAC = function(first_stack, second_stack, angle_stack){
...
anu_stack = rast(anu_list) # himawari-8 lst to be calibrated
...
foreach (t = 1:63, .combine=cbind, .packages=c('terra', 'raster')) %dopar% { # nrow is the number of tiles, which is 63 for now
    ...
    anu_tile = crop(anu_stack, ext_patch)
    mod_tile = crop(mod_stack, ext_patch)
    sza_tile = crop(sza_stack, ext_patch)

    parRst_list = ZAC(anu_tile, mod_tile, sza_tile)
...
writeRaster(par1_aus, paste0(output_path, 'ZAC_v1.4.1_log_par1_2km.tif'), overwrite=TRUE)

par1_aus = raster(paste0(output_path, 'ZAC_v1.4.1_log_par1_2km.tif'))
...
writeRaster(par1_aus, paste0(output_path, 'ZAC_v1.4.1_par1_log_2km_1to99percentile.tif'), overwrite=TRUE)
