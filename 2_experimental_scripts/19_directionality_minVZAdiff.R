...
#              and viirs lst with minimal vza difference
...
# 2. only pixels with vza below 50° should be considered for the bias correction (we did this)
# 3. vza difference between two platforms should be less than 10° (they used 5 °)
    ...
    for (x in 1:NROW){
        ...
        for (y in 1:NCOL){
            ...
            if (length(na.omit(geo_lst_array[x,y,])) < 100) {
                ...
                # drop the rows with large lst/vza diff between two time series
                # drop vza > 50
                dropped_index = which(abs(sub_df$geo_lst - sub_df$polar_lst) > 10 | abs(sub_df$geo_vza - sub_df$polar_vza) > 10 |
                                        sub_df$geo_vza > 50 | sub_df$polar_vza > 50)
...
outpath = paste0('/datasets/work/d61-af-soilmoisture/work/himawari/directionality_par/Nelder_Mead_minVZAdiff/', geo_var)
